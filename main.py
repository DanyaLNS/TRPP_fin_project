    ...
    def send_message(message):
        if (message.text in ['info', 'инфо']):
            bot.send_message(message.chat.id, "Я - бот, созданный студентами ИНБО-07-20! \n"
                                              ...
                                              "Сначала нужно установить язык, для этого отправьте 'ru' или 'en'")
        elif (message.text == 'ru'):
            language = 'ru'
            bot.send_message(message.chat.id, "Установлен русский язык для изменения отправьте 'en'...")
        elif (message.text == 'en'):
            language = 'en'
            bot.send_message(message.chat.id, "Установлен английский язык для изменения отправьте 'ru'...")
        else:
